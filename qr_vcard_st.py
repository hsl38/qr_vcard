...
if __name__ == '__main__':
    ...
    st.markdown('- 스마트폰 카메라로 QR 코드를 스캔하면 화면에 링크가 뜹니다. 링크를 탭하면 연락처 앱에 정보가 입력됩니다.')
    st.markdown('- 종이 명함 대신 사용되어 환경에 도움이 되면 좋겠습니다. 명함 앱 회사에 개인 정보가 저장되는 우려가 없습니다. 사용 후에 새로 고침하세요.')
    st.markdown('- 간략 사용법: 명함 데이터를 입력하고, "QR 코드 생성" 버튼을 클릭하세요. QR 코드 명함 이미지를 다운로드 받으세요. 상세 사용법: https://github.com/hsl38/qr_vcard')
    st.divider()
