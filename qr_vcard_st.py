...
if __name__ == '__main__':
    ...
    st.write('- 입력된 정보나 출력된 QR 코드 명함은 메모리에만 있다가 브라우저를 새로 고침할 때 초기화 됩니다. 명함 앱 회사에 내 개인 정보가 저장되는 것이 꺼림칙해서 만들었습니다. 사용 후에 새로 고침하세요.')
    st.write('- 간략 사용법: 명함 데이터를 입력하고 "QR 코드 생성" 버튼을 클릭하세요. QR 코드 명함 이미지를 다운로드 받으세요.')
